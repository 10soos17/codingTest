...
public class BJ_silver2_12891_DNA비밀번호_투포인터 {
	...
	static int A;static int C;static int G;static int T;
	static String TEXT;
	...
	public static void main(String[] args) throws Exception {
		...
		int l = 0;int r = M;
		for(int i = M; i < N; i++) {
			char beforeS = TEXT.charAt(l);
			char nowS = TEXT.charAt(r);
			int before = map.get(beforeS)-1;
			map.put(beforeS, before);
			int now = map.get(nowS)+1;
			map.put(nowS, now);
			l++;r++;
			if(check()) cnt++;
		}
	...
	static boolean check() {
		...
		int nG = map.get('G');
		int nT = map.get('T');
		if(nA >= A && nC >= C && nG >= G && nT >= T) return true;
		return false;
	}
}
